...
class CompraExcelView(LoginRequiredMixin, CheckPermisosMixin, TemplateView):
    permission_required = "administracion.listar_compra"  # Asegúrate de usar el nombre correcto de tu app

    def get(self, request, *args, **kwargs):
        # Obtener datos con select_related para optimización
        compras = Compra.objects.select_related('articulo').all().values(
        ...
        for compra in compras:
            fecha = compra["created_at"].strftime("%d/%m/%Y %H:%M") if compra["created_at"] else "N/A"
            ws.append([
                compra["articulo__nombre"],

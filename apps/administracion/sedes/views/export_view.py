...
from administracion.sedes.models import Sede, ESTATUS_CHOICES  # Importar el modelo y choices
...
class SedeExcelView(LoginRequiredMixin, CheckPermisosMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for row_num, sede in enumerate(sedes, 3):
            ws.append([
                sede["sede"],
                estatus_map.get(sede["estatus"], "Desconocido"),  # Mapear valor choice
                sede["created_at"].strftime("%d/%m/%Y %H:%M") if sede["created_at"] else "N/A"
            ])

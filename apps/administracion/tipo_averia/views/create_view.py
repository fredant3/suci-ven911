from administracion.tipo_averia.forms import TipoAveriaForm
from administracion.tipo_averia.services import TipoAveriaService
from django.contrib.auth.mixins import LoginRequiredMixin
from django.urls import reverse_lazy
from django.views.generic import CreateView
from helpers.CheckPermisosMixin import CheckPermisosMixin
from helpers.ControllerMixin import CreateController
from templates.sneat import TemplateLayout


class TipoAveriaCreateView(LoginRequiredMixin, CheckPermisosMixin, CreateView):
    permission_required = "administracion.tipo_averia.agregar_tipo_averia"
    form_class = TipoAveriaForm
    template_name = "sneat/layout/partials/form/layout.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context.update(
            {
                "titlePage": "Administración",
                "indexUrl": reverse_lazy("administracion"),
                "module": "Administración",
                "submodule": "Tipos de Averías",
                "titleForm": "Registrar nuevo tipo de avería",
                "tag": "Registrar",
                "listUrl": reverse_lazy("tipo_averias:list"),
                "urlForm": reverse_lazy("api_tipo_averias:create"),
                "methodForm": "POST",
            }
        )
        return TemplateLayout.init(self, context)


class TipoAveriaCreateApiView(CreateController, CheckPermisosMixin):
    permission_required = "administracion.tipo_averia.agregar_tipo_averia"
    form_class = TipoAveriaForm

    def __init__(self):
        self.service = TipoAveriaService()
